...
sed -i 's#delastaging.hypernode.io#'$url'#g' magento-db.sql &&
mysql -uroot -pnem4540 magento2 < magento-db.sql &&
echo "----------------------------------------------"

echo "======= 3. INSTALL MAGENTO ENV & CONFIG ======"
cd $GITPOD_REPO_ROOT/gitpod &&
cp "./.magento.env.php" "$MAGENTO_ROOT/app/etc/env.php" &&
cd $MAGENTO_ROOT &&
...
echo "========== 7. CUSTOM THEME BUILDER (HYVA) ===="
$GITPOD_REPO_ROOT/gitpod/custom-theme-builder.sh
echo "----------------------------------------------"

echo "========== 8. DEPLOY STATIC CONTENT =========="
